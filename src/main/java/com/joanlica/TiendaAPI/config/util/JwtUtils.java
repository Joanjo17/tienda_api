package com.joanlica.TiendaAPI.config.util;
...
public class JwtUtils {
    ...
    public String createToken(Authentication authentication) {
        ...
        Date exp = Date.from(now.plusSeconds(ttlSeconds)); //ExpiresAt

        //a partir de esto generamos el token
        String jwtToken = JWT.create()
                .withIssuer(this.userGenerator) //Generador del Token (Issuer)
                .withSubject(username) // Propietario del Token (el username unico del usuario)
    ...
    public DecodedJWT validateToken(String token) {
        try {
            ...
            JWTVerifier verifier = JWT.require(algorithm)
                    ...
                    .build();
            // Si está bien no da excepción y devuelve el JWT decodificado
